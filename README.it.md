...
- **Lettura** (`get`, `list`) — una riga o molte, filtrabili per stato, referente o priorità, in forma leggibile o `--json`; `get --body`/`--body-only` rilegge anche il corpo della pagina come Markdown.
...
- **Append non distruttivo** (`--append-file` su `upsert`/`set`) — aggiunge Markdown in coda al corpo di una pagina senza toccare ciò che c'è già, in alternativa alla sostituzione di `--body-file`.
...
**Semantica di sostituzione.** `--body-file` **possiede il corpo della pagina**: ogni esecuzione rende il corpo uguale al contenuto del file, cancellando qualunque blocco fosse già presente — incluso ciò che è stato aggiunto a mano in Notion dall'ultima esecuzione. Eseguirlo due volte sullo stesso file produce lo stesso corpo, non un duplicato. Non esiste un annulla, quindi tratta il file come l'unica fonte di verità per quella pagina — ma c'è un modo per vedere cosa un'esecuzione andrebbe a sostituire prima di lanciarla: `get --body` (vedi [Leggere il corpo della pagina](#leggere-il-corpo-della-pagina) più sotto) stampa il corpo attuale senza aprire Notion. Se sostituire tutto è più di quanto serve, `--append-file` (vedi più sotto) aggiunge al corpo invece di sostituirlo. Le sotto-pagine e i database che stanno al primo livello del corpo non vengono mai toccati — vengono saltati invece che archiviati, e un warning su stderr nomina ciascuno di quelli mantenuti. Quel salvataggio riguarda però solo i figli diretti del corpo: una sotto-pagina annidata dentro un toggle, una colonna o un callout se ne va insieme al blocco che la contiene.
...
### `--append-file` — aggiunge al corpo della pagina senza sostituirlo

```
notion-track upsert --ticket <chiave> --append-file note.md
...
**Il limite viene misurato sulla richiesta, non sul file.** `--body-file` viene parsato in blocchi e inviato in più richieste raggruppate, quindi il suo limite di 1 MiB è un totale. Un append viene invece inviato tale e quale come un unico payload `insert_content`, e [Notion limita un payload a 500 KB](https://developers.notion.com/reference/request-limits) — quindi ciò che conta è la dimensione della richiesta serializzata, sempre maggiore del file da cui proviene: il Markdown viene messo in escape dentro una stringa JSON, quindi ogni a capo costa due byte, e `--expand` può aver fatto crescere il testo prima ancora. Un changelog di 450 KB fatto di righe corte può serializzarsi oltre i 500 KB.

Per questo `notion-track` costruisce il payload e lo misura, dopo l'espansione, prima di fare qualunque chiamata. Oltre il limite è exit code 2 con indicata la dimensione della richiesta; un append non si può spezzare, quindi va diviso in due esecuzioni oppure si usa `--body-file`.
...
**Non è idempotente.** A differenza di `--body-file`, eseguire due volte `--append-file` con lo stesso contenuto lo aggiunge due volte — non esiste un controllo "già presente". Tienilo a mente se lo inserisci in un ciclo di retry.
...
### Leggere il corpo della pagina

```
notion-track get --ticket <chiave> --body
notion-track get --page-id <id> --body-only > note.md
```

`--body` e `--body-only` aggiungono il corpo della pagina, come Markdown, a ciò che `get` stampa — e funzionano con tutte e tre le forme di indirizzamento, `--ticket`, `--id` o `--page-id`. `--body` stampa la riga come al solito, poi il corpo sotto; `--body-only` stampa **solo** il Markdown, nient'altro, così `notion-track get --ticket TASK-231 --body-only > note.md` produce un file Markdown valido invece di uno con un riepilogo della riga incollato in cima. `--body` e `--body-only` sono mutuamente esclusivi.

Con `--json`, `--body` annida la forma della riga già esistente sotto `page` e aggiunge il corpo sotto `body`: `{"page": {...}, "body": {"markdown": "...", "truncated": false, "unknown_block_ids": []}}`. Senza `--body`, il JSON resta quello di prima di questa funzionalità — piatto, senza il wrapper `page`/`body` — così gli script esistenti che leggono `get --json` continuano a funzionare senza modifiche. `--body-only --json` stampa il solo oggetto `body`, senza wrapper, coerentemente con quello che `--body-only` fa per la forma leggibile: il corpo e nient'altro.

**`truncated` e `unknown_block_ids`.** Notion tronca il corpo di una pagina intorno ai 20.000 blocchi; `truncated: true` significa che il Markdown è reale ma incompleto, non che qualcosa sia andato storto. `unknown_block_ids` elenca i blocchi che Notion non riesce proprio a rendere come Markdown — un bookmark, un embed, un'anteprima di link, una breadcrumb, un pulsante template — che nel Markdown compaiono come `<unknown/>`. Entrambi i casi stampano un warning su stderr (mai su stdout, così `--body-only > note.md` resta pulito). Nessuno dei due significa che il contenuto sia andato perso: un blocco `<unknown/>` è ancora sulla pagina, esattamente come prima; è solo la sua esportazione testuale a non esistere.

**Leggere serve per ispezionare, non per un ciclo di andata e ritorno.** Il Markdown restituito da `get --body` non è garantito riprodurre la pagina se rimandato indietro tramite `--body-file`: gli URL dei file e delle immagini caricati nel Markdown restituito sono firmati da Notion a tempo e scadono, e un blocco `<unknown/>` non ha comunque una forma Markdown da riscrivere. Usa `--body`/`--body-only` per vedere cosa c'è su una pagina — incluso, come nella nota sopra, per controllare cosa un'esecuzione di `--body-file` andrebbe a sostituire prima di lanciarla — non per scaricare una pagina, modificare il file e ricaricarlo come flusso di lavoro abituale.
...
| `5` | Auth | nessun token trovato (incluso il caso in cui `credentials.yml` esiste ma non è leggibile), oppure Notion lo ha rifiutato (401/403) — incluso `doctor`, quando il suo check `token` è l'unico fallito. Anche `--assignee me` esce con 5 quando `credentials.yml` non è leggibile: un'identità potrebbe essere lì dentro, e ripiegare sul `me:` condiviso del profilo assegnerebbe la riga a qualcun altro |
...
Implementato oggi: `init` (procedura guidata interattiva e forma a flag, con `--list`), la TUI di navigazione, `upsert`, `set`, `get`, `list`, `doctor`; `--dry-run` su `upsert`/`set`; `apply` per le scritture in blocco da manifest; `--body-file` su `upsert`/`set` per scrivere il corpo della pagina da Markdown, con `--expand` per i segnaposto `{{ticket}}`/`{{date}}`; `--append-file` su `upsert`/`set` per aggiungere Markdown in coda al corpo di una pagina senza sostituirlo; `get --body`/`--body-only` per rileggere il corpo di una pagina come Markdown; `--json` su ogni comando che produce output; `mcp` per servire le stesse operazioni come tool MCP; un ruolo `assignee` opzionale con `--assignee`/`--unassign`, `list --assignee`/`--unassigned` e l'identità `me`; un ruolo `priority` opzionale con `--priority` su `upsert`/`set`/`list`; un ruolo `id` opzionale mappato con `init --id-prop`, che indirizza una riga tramite il suo id di board con `--id` su `get`/`set`; profili; retry con backoff.

Automazione di release: la **pipeline GoReleaser** (`.goreleaser.yaml` più un workflow di release che parte sui tag `v*`) compila e pubblica i binari e il `checksums.txt` descritti in [Installazione](#installazione) a ogni tag spinto. La **composite GitHub Action** in [`action/`](action/) installa dalla release a cui risolve il suo input `version` (vedi [Uso in CI](#uso-in-ci)). Entrambe vengono esercitate, in due punti. Il job `smoke` del workflow di release installa con la action ogni release appena pubblicata, su Linux, macOS e Windows — così i nomi degli archivi che GoReleaser scrive vengono confrontati con quelli che la action ricostruisce, a ogni release. Separatamente, il workflow `action-smoke` installa la release corrente a ogni modifica dell'action, una volta a settimana contro le immagini dei runner che cambiano sotto, e attraverso il riferimento `marcoarnulfo/notion-cli/action@main` che questo README documenta. Due archivi pubblicati restano fuori da entrambi: `windows/arm64` non ha un runner ospitato da GitHub su cui installarlo, e `darwin/amd64` non ha una riga in nessuna delle due matrici — tracciato nella [#30](https://github.com/marcoarnulfo/notion-cli/issues/30).
