...
Automazione di release: la **pipeline GoReleaser** (`.goreleaser.yaml` più un workflow di release che parte sui tag `v*`) compila e pubblica i binari e il `checksums.txt` descritti in [Installazione](#installazione) a ogni tag spinto. La **composite GitHub Action** in [`action/`](action/) installa dalla release a cui risolve il suo input `version` (vedi [Uso in CI](#uso-in-ci)). Il workflow `action-smoke` lega le due cose: installa una release pubblicata su runner Linux, macOS e Windows — anche attraverso il riferimento `marcoarnulfo/notion-cli/action@main` che questo README documenta — a ogni modifica dell'action, a ogni release pubblicata e una volta a settimana contro le immagini dei runner da cui l'action dipende. Due archivi pubblicati restano fuori: `windows/arm64` non ha un runner ospitato da GitHub su cui installarlo, e `darwin/amd64` non ha una riga nella sua matrice.
